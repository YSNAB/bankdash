  ...
  const fetchOrder = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const orderItems = data.orderDetails.map((detail: OrderDetail) => ({
        ...
        setItems(orderItems)
        setSearchQuery(orderItems.map((item: OrderItem) => item.productName))
        setFilteredProducts(orderItems.map(() => []))
      } else {
      ...
      <body>
        ...
        <div class="totals">
          ...
          <div class="totals-row">
            ...
            <div class="totals-value">${formatPrice(parseFloat(paidAmount))}</div>
          </div>
          <div class="totals-row" style="font-weight: bold; color: ${isPaid ? '#22c55e' : '#ef4444'};">
            <div class="totals-label">Balance:</div>
            <div class="totals-value">${formatPrice(parseFloat(total) - parseFloat(paidAmount))}</div>
          </div>
        </div>
                ...
                <div key={index} className="flex gap-4 items-end p-5 backdrop-blur-xl bg-white/40 dark:bg-slate-800/40 border border-white/30 dark:border-slate-700/30 rounded-2xl">
                  <div className="flex-1 relative">
                    ...
                    {filteredProducts[index] && filteredProducts[index].length > 0 && (
                      <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                        {filteredProducts[index].map((product) => (
                          <button
                            key={product.id}
                            type="button"
                            onClick={() => selectProduct(index, product)}
                            className="w-full px-4 py-2 text-left text-sm hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-900 dark:text-white transition-colors"
                          >
